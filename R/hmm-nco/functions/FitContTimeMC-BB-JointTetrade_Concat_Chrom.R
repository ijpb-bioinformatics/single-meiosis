...
SplitEMsame <- function(EMsameConcat,Chrom,DataName){
	tmp=EMsameConcat	
	for(i in 1:5){
		print(i)
	...
	EMsame=tmp
	save(EMsame,file=paste(ResDir, "/",DataName,"-","Chr",i,'-JointCHMM-SameRates.Rdata', sep=''))
}
}
...
Chrom0 <- rep("Concat0",length(save_Chrom))
Chrom0B<-rep(paste("Chr",Concat0,sep=""),nbMarqByChrom[Concat0]) 

Pos1<-rep(NA,length(data.M$Chrom))
Pos0<-rep(NA,length(data.M$Chrom))
for(i in 1:5){
...
# }
 for(i in 1:4){
 newtetrade[[i]]$ID=as.factor(c(paste(Chrom0B,Pos0,sep="_")))
 newtetrade[[i]]$Chrom=as.factor(c(Chrom0))
 newtetrade[[i]]$Pos=c(Pos0)
 newtetrade[[i]]$R=c(tetrade[[i]]$R)
 newtetrade[[i]]$Y=c(tetrade[[i]]$Y)
 newtetrade[[i]]$Ind=c(tetrade[[i]]$Ind)
 }
...
for (c in  1:Chr.nb){
  ...
  SplitEMsame(EMsame,Chrom0B,DataName)	
}
