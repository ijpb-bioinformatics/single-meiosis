...
load("Tetrads_genotype_sample_30000.Rdata")

CO_data <- read.table("CO.txt",h=TRUE,sep="\t") 

NCO_data <- read.table("NCO.txt",h=TRUE,sep="\t")
...
Score <- read.table("HMM_MAP_ByTetrads.txt", h=TRUE)
Score$Genotype <- factor(Score$Genotype, 
                         levels = c("wt","Fa","Fi","FiFa","QAB","QFa","QFi","QFiFa"))

load("ColGenomeFeatures.Rdata")
...
### Alignment, filtering and count: Pipeline single-meiosis.sh [author: Charif D. et Tran J.]
...
-   Alignment were filtered out to remove non specific hit and PCR
    ...
    that are specific to a parental genome but also specific to a
    location within a genome. This parameters discards reads that span
    conversion but allowing mismatches introduced two much noises in       the results. This settings was the best compromise between             specificity and sensitivity.
...
VCF files were crossed with the allelique SNVs list between Col-0 and Ler-0 provided by @Zapata2016 (TabOfSNP.txt) and serve as input to the HMM.

### Individuals and Tetrad genotyping: HMM_model R functions [author: Robin S.]

An HMM model was implemented by S. Robin to obtain both the individuals and tetrad genotypes at each SNV position from the 4 count files (M1, M2, M3, M4) of a Tetrad. 
...
Because of the experimental setting, at every marker, the ratio
...
-   The hidden states of each individuals have been considered jointly.
...
-   To take into account the non homogeneous distance separating the
    ...
    states to another are given by transition rates $$
...
event. Two COs could be detected if they are at least distant by 5kb.

### NCOs detection:

NCOs correspond to markers with the following states
$St=\{2,3,5,8,9,12,14,15\}$ Successiv markers with a genotype
corresponding to one of this states and with a maximum distance of 2 Kb
between each belong to the same event.
...
2.  SNVs where then filtering according to the followig criterions:

-   Removing of SNVs associated to COs that are found in several tetrads
    (file Liste_FAUX_CO.txt)
-   Removing of markers with non 2:2 (fichier df_Genotype_SNP_count.txt)
...
used a bed file from personal communication with Dr. Mezard.
...
```{r}
sequencing <- read.table("RawReadsCount.txt",h=TRUE) |>
...
```{r design}
table(unique(read.table("TetradsProj2TetradsPaper.txt",h=TRUE))$Genotype)
```
...
The score is positively correlated to the rate of true positiv NCO

More the tetrad’s depth is important, less the number of event to check
is high and the highest is the rate of TP NCO.

```{r scorePlot, width=5, height=5}
...
pGCCO <- ggplot(GC.nbSNP) + 
    ...
    scale_fill_manual(values = c("red", "darkred"), name = "", 
...
fig2 <- grid.arrange(pCO,pNCO,pCONCO,pGCCO, ncol = 2, nrow=2)
fig2

ggsave(filename="fig2.pdf", plot=fig2)
...
Ici, je n'ai pas appliqué ce filtre ... alors les résultats diffèrent un
peu.
...
```{r NCOdesc, , caption="table1"}
NCO_data %>% group_by(Genotype) %>% dplyr::summarise(n=n(),median.mean=mean(medianD),minTL.mean=mean(minTL),maxTL.mean=mean(maxTL))
```
#### CO

```{r COdesc, caption="table1"}
CO_data_bis <- read.table("CO_Description.txt",h=TRUE,sep="\t") 
CO_data_bis$Genotype <- factor(CO_data_bis$Genotype, levels = c("wt", "Fa","Fi","FiFa","QAB","QFa","QFi","QFiFa"))
CO_data_bis %>% filter(is.gc==TRUE) %>% group_by(Genotype) %>% dplyr::summarise(n=n(),median.mean=mean(medianD),minTL.mean=mean(minTL),maxTL.mean=mean(maxTL))
```

#### Comparison of CO and NCO TL within Genotype

```{r}
# Supposons que vos données combinées soient déjà dans `TL.NCO` et `TL.CO`

# Combiner les données si ce n'est pas déjà fait
data <- rbind(TL.NCO, TL.CO)

# Effectuer le test de Wilcoxon pour chaque Genotype
wilcox_wtOther_co <- data |>
  dplyr::group_by(Genotype) |>
  dplyr::filter(dplyr::n_distinct(Type) == 2) |> # S'assurer qu'il y a bien des valeurs pour CO et NCO
  tidyr::nest() |>  # Créer des sous-ensembles pour chaque groupe de Genotype
...
HeteroOCC<-as.matrix(c( table(GCCOannot$is.Hetero[-which(COannot$is.TE==TRUE)])[2],
...
codingOcc<-as.matrix(c(table(GCCOannot$is.Hetero[-which(COannot$is.TE==TRUE)])[2],
                       table(COannot$is.Coding)[2],
                       table(NCOannot$is.Coding)[2]))
dimnames(codingOcc)[[1]]=NULL
dimnames(codingOcc)[[2]]="Coding"

Occ <- base::cbind(utr3Occ, utr5Occ ,intronOcc, codingOcc,TEOcc,HeteroOCC)
...
Occ.event <- base::cbind(UTR3.event, UTR5.event, Intron.event,codingOcc.event,TEOcc.event,HeteroOCC.event)
...
Compartiments <- c("Introns","Exons","5'UTR","3'UTR","Centromere","Heterochromatin","TE")
SumLength <- c(SumIntronsLength,SumExonsLength,SumfiveUTRLength,SumthreeUTRLength,SumCentroLength,SumHeteroLength,SumTELength)
NbSNP <- as.vector(c(NbSNPinIntrons$n,NbSNPinExons$n,NbSNPinFiveUTR$n,NbSNPinThreeUTR$n,NbSNPinCentro$n,NbSNPinHetero$n,NbSNPinTE$n))
...
Features.df$Compartiments <- c("Intron","Coding","UTR5","UTR3","Hetero","TE")
...
p3  <- ggplot(data_long, aes(x = features, y = Proportion, fill = Measure)) +
  ...
  labs(x = "Features", y = "Proportion", fill = "Type",
       title = "SNP distribution in features compared to the compartiment's size of features") + 
...
p4 <- ggplot(disSNP_res_long, aes(x = Features, y = Count, fill = Type)) +
  geom_bar(stat = "identity", position = "dodge") +  
  labs(x = "Features", y = "Number of converted SNP (sum of all tetrads)", fill = "Type") +  
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10)) +  
...
```{r tlNCOres, echo=FALSE, caption="sup_figure3", width=5, height=5}
load("manipTracLengthNCO.RData")
